  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-mono text-lg font-semibold text-foreground">{t("dashboard.projects")}</h1>
        {editingId === null && (
        ...
        {items.map((p) => (
          <Card key={p.id} className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="font-mono text-xs text-foreground-muted">{p.category || "—"}</p>
            </div>
            <div className="flex shrink-0 gap-2">
              <button onClick={() => startEdit(p)} className="p-2 text-foreground-muted hover:text-foreground">
                <PencilIcon className="size-4" />
              </button>
              <button onClick={() => handleDelete(p.id)} className="p-2 text-foreground-muted hover:text-danger">

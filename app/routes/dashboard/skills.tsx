...
import { PencilIcon, TrashIcon, PlusIcon, QuestionMarkCircleIcon } from "@heroicons/react/24/outline";
...
type Skill = { id: number; name: string; icon_key: string; category: string; sort_order: number };
type FormState = Omit<Skill, "id">;

const emptyForm: FormState = { name: "", icon_key: "", category: "", sort_order: 0 };
...
function IconPreview({ iconKey }: { iconKey: string }) {
  ...
  if (!iconKey) return <QuestionMarkCircleIcon className="size-5 text-foreground-muted" />;
  if (!Icon) return <span className="font-mono text-xs text-danger">not found</span>;
  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-mono text-lg font-semibold text-foreground">{t("dashboard.skills")}</h1>
        {editingId === null && (
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {items.map((s) => (
          <Card key={s.id} className="flex flex-col items-center gap-2 text-center">
            <IconPreview iconKey={s.icon_key} />
            <p className="text-sm text-foreground">{s.name}</p>
            <div className="flex gap-2">
              <button onClick={() => startEdit(s)} className="p-1 text-foreground-muted hover:text-foreground">
                <PencilIcon className="size-4" />
              </button>
              <button onClick={() => handleDelete(s.id)} className="p-1 text-foreground-muted hover:text-danger">
